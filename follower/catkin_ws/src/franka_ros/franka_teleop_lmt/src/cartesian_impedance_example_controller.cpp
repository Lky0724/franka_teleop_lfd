...
bool CartesianImpedanceExampleController::init(hardware_interface::RobotHW* robot_hw,
	...
	typedef actionlib::SimpleActionClient<franka_gripper::HomingAction> HomingClient;
	...
	*F_Pub = node_handle.advertise<std_msgs::Float64MultiArray>("FLfeedback", 1,true);
	...
	fprintf(fidRecord, "Vf_x\tVf_y\tVf_z\t");    	// Follower Velocity
	fprintf(fidRecord, "Xf_x\tXf_y\tXf_z\t");    	// Follower Position
	fprintf(fidRecord, "Rf_w\tRf_x\tRf_y\tRf_z\t");    	// Follower Orientation
	fprintf(fidRecord, "Fc_x\tFc_y\tFc_z\t");  		// Follower Force
	fprintf(fidRecord, "Gr\t");  					// Gripper state (0/1)
	...
	cartesian_damping_.topLeftCorner(3, 3) << 3.0 * sqrt(translational_stiffness_) * Eigen::Matrix3d::Identity();
	cartesian_damping_.bottomRightCorner(3, 3) << 3.0 * sqrt(rotational_stiffness_) * Eigen::Matrix3d::Identity();

	ROS_INFO("INIT CONTROLLER SUCCESSFUL");
	return true;
}
...
void CartesianImpedanceExampleController::starting(const ros::Time& /*time*/) {
	...
	ROS_INFO("START CONTROLLER SUCCESSFUL");
...
void CartesianImpedanceExampleController::update(const ros::Time& time,
	...
	/////////////////////////////////////////////////////////////////////////////
	// TO-DO: Read the desired cartesian velocity and rotation command and set target position and orientation
	/////////////////////////////////////////////////////////////////////////////
	ROS_INFO_THROTTLE(1.0, "ROBOT CONTROL LOOP ACTIVE");  // 1 Hz, safer in RT
	...
	if(command_.size()==3){ // only cartesian velocity commands
	...
	if(command_.size()==7){ // 3 cartesian velocity + 3 euler angle error commands + 1 gripper command (not used here)
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%*/
			position_d_ = position_d_ + period.toSec() * Eigen::Vector3d(command_[0], command_[1], command_[2]);
			orientation_e_ = Eigen::Vector3d(command_[3], command_[4], command_[5]);
			
			// Small-angle command (radians). Interpret as an axis-angle vector r = [rx, ry, rz]
			has_euler_err_cmd = true;
			Eigen::Vector3d r(command_[3], command_[4], command_[5]);
			double theta = r.norm();

			Eigen::Quaterniond q_delta = Eigen::Quaterniond::Identity();
			if (theta > 1e-9) {
				q_delta = Eigen::AngleAxisd(theta, r / theta);
			} else {
				// small-angle approx: q ≈ [1, 0.5*r]
				q_delta = Eigen::Quaterniond(1.0, 0.5*r.x(), 0.5*r.y(), 0.5*r.z());
			}

			// Apply in desired frame:
			// - If r is in EE (body) frame:
			orientation_d_ = orientation_d_ * q_delta;
			// - If r is in base/world frame, use instead:
			// orientation_d_ = q_delta * orientation_d_;

			orientation_d_.normalize();

			gripper_state = command_[6];
	...
	//////////////////////////////////////////////////
	// Comment this part if impedance control for rotation is desired.
	// Old code for rotation: directly write euler angle command to error.tail(3).
	// This disables the impedance control for rotation since orientation_d_ won't be updated,
	// but it is more robust when orientation command is sent to through haptic buttons and
	// real accurate desired orientation can not be given by leader, and when there is
	// external force from the environment acting on robot.
	// If this part is used, the command should be larger, like 0.01, rather than 0.0005.
	
    if (has_euler_err_cmd) {
		double theta = orientation_e_.norm();
		Eigen::Quaterniond error_quaternion = Eigen::Quaterniond::Identity();
		if (theta > 1e-9){
			Eigen::Vector3d axis = orientation_e_ / theta;
			error_quaternion = Eigen::AngleAxisd(theta, axis);
		}
		else{
			// small-angle approx: q ≈ [1, 0.5*r]
			error_quaternion = Eigen::Quaterniond(1.0, 0.5*orientation_e_.x(),
													   0.5*orientation_e_.y(),
													   0.5*orientation_e_.z());
		}
        // If euler_err_cmd is in EE frame, keep the transform:
        error.tail(3) << error_quaternion.x(), error_quaternion.y(), error_quaternion.z();
	...
	orientation = Eigen::Quaterniond(transform.linear()).normalized();
...
void CartesianImpedanceExampleController::RecordSignals(const ros::TimerEvent&) {
  ...
  if (DataArrived == true) {
	...
	fprintf(fidRecord, "%f\t%f\t%f\t%f\t", tmp->Rf[0], tmp->Rf[1], tmp->Rf[2], tmp->Rf[3]);
    fprintf(fidRecord, "%f\t%f\t%f\t", tmp->Fc[0], tmp->Fc[1], tmp->Fc[2]);
	fprintf(fidRecord, "%f\t", tmp->Gr);
		fprintf(fidRecord, "%d\n", tmp->first_packet);
  }
  fclose(fidRecord);
}
