...
int rotation_axis = 0; // axis of rotation command sent
...
double orientation_command[3] = {0.0, 0.0, 0.0}; // Delta Euler angle for orientation command
int gripper_command = 0;                   // Gripper command
...
void* HapticsLoop(void* ptr) {
  ...
  if (feedbackMutex.try_lock()) {
    ...
    %%% Your code here!
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%*/
    double factor = 3.0; // force feedback scaling factor
    if (Fl_norm > 20.0 | dhdGetButtonMask() == 3){ // stop force feedback if F > 20N or buttons 1&2 are pressed
      if (dhdGetButtonMask() == 3){
        ROS_INFO("Pressed button 1 & 2, force feedback stopped");
      }
      else{
        ROS_INFO("High feedback force detected: %f, %f, %f, force feedback stopped", Fl[0], Fl[1], Fl[2]);
      }
      Fl[0] = 0;
      Fl[1] = 0;
      Fl[2] = 0;
    ...
    orientation_command[1] = 0.0;
    orientation_command[2] = 0.0;
    button_pressed = 0;

    command.insert(command.end(), Vl, Vl+3);

    //////////////////////////////////////////////////////////
    // workspace scaling tuning based on keyboard input
    //////////////////////////////////////////////////////////
    if (kbhit()) {
      int key = 2;
      char c = getchar();
      ...
      case 1: 
        ws = 1;
        ROS_INFO("ws is set as 1");
        break;
      case 2: 
        ws = 2;
        ROS_INFO("ws is set as 2");
        break;
      case 3: 
        ws = 3;
        ROS_INFO("ws is set as 3");
        break;
      case 4: 
        ws = 4;
        ROS_INFO("ws is set as 4");
        break;
      case 5: 
        ws = 5;
        ROS_INFO("ws is set as 5");
        break;
      case 6: 
        ws = 6;
        ROS_INFO("ws is set as 6");
        break;
      case 7: 
        ws = 7;
        ROS_INFO("ws is set as 7");
        break;
      case 8: 
        ws = 8;
        ROS_INFO("ws is set as 8");
        break;
      case 9: 
        ws = 9;
        ROS_INFO("ws is set as 9");
        break;
    }
    }
    ...
    // haptic buttons
    //////////////////////////////////////////////////////////
    button_pressed = dhdGetButtonMask();
    switch (button_pressed){
      case 1: // hold follower position, for workspace adjustment
        command[0] = 0.0;
        command[1] = 0.0;
        command[2] = 0.0;
        ROS_INFO("Pressed button 1, hold robot position");
        break;
      case 4: // switch rotation axis
        if (last_button_pressed != button_pressed) {
          rotation_axis += 1;
          if (rotation_axis > 2) rotation_axis = 0;
        }
        ROS_INFO("Pressed button 4, switch rotation axis to %d", rotation_axis);
        break;
      case 2: // positive rotation
        // orientation_command[rotation_axis] = 0.0005;  // if orientation is using impedance control
        orientation_command[rotation_axis] = 0.6;  // if orientation is not using impedance control
        ROS_INFO("Pressed button 2, positive rotation about axis %d", rotation_axis);
        break;
      case 8: // negative rotation
        // orientation_command[rotation_axis] = -0.0005;  // if orientation is using impedance control
        orientation_command[rotation_axis] = -0.6;  // if orientation is not using impedance control
        ROS_INFO("Pressed button 8, negative rotation about axis %d", rotation_axis);
        break;
      case 5: // gripper command
        if (last_button_pressed != button_pressed) {
          gripper_command = 1 - gripper_command;
        }
        ROS_INFO("Pressed buttons 1 & 4, gripper command: %d", gripper_command);
        break;
    ...
    //     ROS_INFO("Pressed button 1, hold robot position");
    //     break;
    //   case 2: // rotation about x
    //     orientation_command[0] = 0.1 * rotate_direction;
    //     ROS_INFO("Pressed button 2, rotate about x");
    //     break;
    //   case 4: // rotation about y
    //     orientation_command[1] = 0.1;
    //     ROS_INFO("Pressed button 4, rotate about y");
    //     break;
    //   case 8: // rotation about z
    //     orientation_command[2] = 0.1;
    //     ROS_INFO("Pressed button 8, rotate about z");
    //     break;
    //   case 5:
    //     rotate_direction = -rotate_direction;
    //     ROS_INFO("Pressed buttons 1 & 4, change rotation direction");
    //   case 10:
    //     // gripper command
    //     ROS_INFO("Pressed buttons 2 & 8, gripper command");
    //     break;
    }

    command.insert(command.end(), orientation_command, orientation_command+3);
    command.push_back(gripper_command);
...
int kbhit() {
    ...
    newt = oldt;
    newt.c_lflag &= ~(ICANON | ECHO); // Raw mode: no Enter, no Echo
    tcsetattr(STDIN_FILENO, TCSANOW, &newt);
    oldf = fcntl(STDIN_FILENO, F_GETFL, 0);
    fcntl(STDIN_FILENO, F_SETFL, oldf | O_NONBLOCK); // Don't freeze
    ...
    if(ch != EOF) {
        ungetc(ch, stdin); // Put the character back to be read by getchar()
        return 1;
    }
    return 0;
}
